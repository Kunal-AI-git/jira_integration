...
def search_issues(jql: str):
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=f"JIRA API error with JQL '{jql}': {str(e)}"
        )

@app.get("/stories/{story_id}/tasks", summary="Fetch tasks for a story")
def fetch_subtasks(story_id: str):
    auth, headers = get_jira_session()
    url = f"{JIRA_BASE_URL}/rest/api/3/issue/{story_id}"
    issue = safe_request("GET", url, headers, auth)
        ...
        for epic in epics:
            ...
            try:
                ...
                for story in stories:
                    subtasks = fetch_subtasks(story["id"])
                    task_models = [
                ...
                print(f"[WARN] Failed to fetch stories/subtasks for epic {epic['key']}: {e}")
                continue

            epics_data.append(EpicModel(
                id=str(epic["id"]),
                name=epic.get("name") or epic.get("summary", "Unnamed Epic"),
                stories=stories_data
            ))
        boards_data.append(BoardModel(id=board["id"], name=board["name"], epics=epics_data))
    return boards_data
